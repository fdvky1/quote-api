...
const lighten = (color, amount) => {
  ...
  color = `#${addLight(color.substring(0, 2), amount)}${addLight(
    color.substring(2, 4),
    amount
...
module.exports = async (parm) => {
  ...
  if (quoteImages.length > 1) {
    ...
    const canvas = createCanvas(
      width,
      height + quoteMargin * quoteImages.length
    );
    const canvasCtx = canvas.getContext("2d");
  ...
  if (type === "quote") {
    ...
    } else if (canvasPadding.height >= canvasPadding.width)
    ...
    const canvasPic = createCanvas(
      canvasImage.width + padding * 1.7,
      canvasImage.height + padding * 1.7
    );
    const canvasPicCtx = canvasPic.getContext("2d");

    canvasPicCtx.fillStyle = lighten(parm.backgroundColor, 20);
    canvasPicCtx.fillRect(
      0,
      0,
      canvasPic.width + padding,
      canvasPic.height + padding
    );
