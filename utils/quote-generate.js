...
class QuoteGenerate {
  ...
  async downloadAvatarImage(user) {
    ...
    if (user.first_name && user.last_name)
      nameLatters = runes(user.first_name)[0] + runes(user.last_name || "")[0];
    else {
      ...
      if (nameWord.length > 1)
        nameLatters = runes(nameWord[0])[0] + runes(nameWord.splice(-1)[0])[0];
    ...
    if (avatarImageCache) {
      ...
      try {
        ...
        if (user.photo && user.photo.big_file_id)
          userPhotoUrl = await this.telegram
            .getFileLink(user.photo.big_file_id)
            .catch(console.error);

        if (!userPhotoUrl) {
          const getChat = await this.telegram
            .getChat(user.id)
            .catch(console.error);
  ...
  async downloadMediaImage(media, mediaSize, type = "id", crop = true) {
    ...
    if (mediaUrl.match(/.tgs/)) {
      ...
      if (imageMetadata.format === "webp") {
        ...
        croppedImage = await jimpImage
          .autocrop(false)
          .getBufferAsync(Jimp.MIME_PNG);
      } else {
        const smartcropResult = await smartcrop.crop(sharpPng, {
          width: mediaSize,
          height: imageMetadata.height,
        });
        const crop = smartcropResult.topCrop;

        croppedImage = imageSharp.extract({
          width: crop.width,
          height: crop.height,
          left: crop.x,
          top: crop.y,
  ...
  async drawMultilineText(
    ...
    if (entities && typeof entities === "object") {
      for (let entityIndex = 0; entityIndex < entities.length; entityIndex++) {
        ...
        for (
          let charIndex = entity.offset;
          charIndex < entity.offset + entity.length;
          charIndex++
        ) {
          styledChar[charIndex].style =
            styledChar[charIndex].style.concat(style);
        }
      }
    }
    ...
    for (let emojiIndex = 0; emojiIndex < emojis.length; emojiIndex++) {
      ...
      for (
        let charIndex = emoji.offset;
        charIndex < emoji.offset + emoji.length;
        charIndex++
      ) {
    ...
    for (let index = 0; index < styledChar.length; index++) {
      ...
      if (
        ...
        ((charStyle.emoji && !lastChar.emoji) ||
          ...
          (charStyle.emoji &&
            lastChar.emoji &&
            charStyle.emoji.index !== lastChar.emoji.index) ||
          charStyle.char.match(breakMatch) ||
          (charStyle.char.match(spaceMatch) &&
            !lastChar.char.match(spaceMatch)) ||
          (lastChar.char.match(spaceMatch) &&
            !charStyle.char.match(spaceMatch)) ||
          (charStyle.style &&
            lastChar.style &&
      ...
      if (!styledWords[stringNum]) {
        ...
        if (charStyle.customEmojiId)
          styledWords[stringNum].customEmojiId = charStyle.customEmojiId;
    ...
    if (getCustomEmojiStickers) {
      for (let index = 0; index < getCustomEmojiStickers.length; index++) {
        ...
        loadCustomEmojiStickerPromises.push(
          (async () => {
            const getFileLink = await this.telegram
              .getFileLink(sticker.thumb.file_id)
              .catch(() => {});
            ...
            if (getFileLink) {
              ...
              customEmojiStickers[sticker.custom_emoji_id] = await loadImage(
                sharpPng
              ).catch(() => {});
            }
          })()
    ...
    for (let index = 0; index < styledWords.length; index++) {
      ...
      if (styledWord.emoji) {
        if (
          styledWord.customEmojiId &&
          customEmojiStickers[styledWord.customEmojiId]
        ) {
      ...
      if (
        ...
        while (
          ...
          styledWord.word = styledWord.word.substr(
            0,
            styledWord.word.length - 1
          );
      ...
      if (
        styledWord.word.match(breakMatch) ||
        (lineWidth > maxWidth - fontSize * 2 && wordlWidth < maxWidth)
      ) {
        if (
          styledWord.word.match(spaceMatch) &&
          !styledWord.word.match(breakMatch)
        )
          styledWord.word = "";
        if (
  ...
  async drawQuote(
    ...
    if (replyName) {
      ...
      canvasCtx.drawImage(
        this.deawReplyLine(
          3 * scale,
          replyName.height + replyText.height * 0.4,
          lineColor
        ),
        textPosX - 7,
        replyNamePosY
  ...
  async generate(
    ...
    if (message.media) {
      ...
      mediaCanvas = await this.downloadMediaImage(
        media,
        maxMediaSize,
        type,
        crop
